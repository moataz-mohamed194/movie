...
class ValidationBloc{
  ...
  Stream<String> get repeatPassword =>
      _repeatPasswordController.stream.transform(validateRepeatPassword).doOnData((event) {
        if(0 != _passwordController.value.compareTo(event)){
          _repeatPasswordController.addError('password not same');
        }
      });

  Stream<bool> get loginValidation => Rx.combineLatest2(email, password, (e, p) => true);
  Stream<bool> get signUpValidation => Rx.combineLatest4(name,email, password,repeatPassword, (e, p,t,y) => true);
  ...
  StreamTransformer<String, String>.fromHandlers(handleData: (email, sink) {
    bool emailValid = RegExp(r"([a-zA-Z0-9_\-\.]+)@([a-zA-Z0-9_\-\.]+)\.([a-zA-Z]{2,5})").hasMatch(email);
  ...
  var validatePassword = StreamTransformer<String, String>.fromHandlers(
      handleData: (password, sink) {
        ...
        if (passwordValid0 == true &&
            passwordValid1 == true &&
            passwordValid2 == true &&
            passwordValid3 == true && password.length > 8) {
  ...
  StreamTransformer<String, String>.fromHandlers(handleData: (repeatPassword, sink) {
  ...
  signUp(context){
    FirebaseDatabase.instance
      ...
      FirebaseAuth.instance.createUserWithEmailAndPassword(email: _emailController.value, password: _passwordController.value).whenComplete(()  {
        ...
        Toast.show("Login done", context, duration: Toast.LENGTH_SHORT, gravity:  Toast.BOTTOM);

        print("Done");
      });
    });
    print("signUp ${_nameController.value}:${_emailController.value}:${_passwordController.value}:${_repeatPasswordController.value}");
  }

  login(context) async {
    print("ddddddddddddddddddddddddddd");
    FirebaseAuth.instance.signInWithEmailAndPassword(email: _emailController.value, password: _passwordController.value).whenComplete(() {
